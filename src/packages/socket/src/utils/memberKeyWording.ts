 ...
 * looking wrong. A boundary off by one says "same key for 0 days"; a sentence
 * that picks a cause tells somebody their peer got a new phone when a server
 * swapped a key. Neither fails to compile and neither looks odd in the source.
 *
 * `check-member-key-wording.mjs` walks the boundaries.
 */

/**
 * When a key was pinned, in words rather than a date nobody reads.
 *
 * "Since 3 August" answers the question this is for — how long has this been
 * the same key — better than a timestamp does, and how long is the whole of it:
 * a key seen once yesterday and a key seen every day for a year both verify,
 ...
 *
 * The honest cause is unknown from here. Somebody restoring a different seed
 * and a server substituting a key produce the same event, and only a person can
 * tell them apart — usually by asking, through some channel this server is not
 * on. So this says what moved and what it would mean, and stops.
 */
